...
其实写这个东西是用来练手GPU计算，顺便挖掘一些好玩的东西，然后发现CUDA有太多好玩的东西了~
...
```powershell
python Tree_plot\main.py
```

查看所有参数：

```powershell
python Tree_plot\main.py --help
```

只做 Tree(3) 计数演示，不打开画图窗口：

```powershell
python Tree_plot\main.py --sum-node 7 --count-only
```

演示大节点数 GPU 计数：

```powershell
python Tree_plot\main.py --sum-node 200 --count-only
```

生成最多 200 个节点的随机树，并打开可缩放、可拖动的图：

```powershell
python Tree_plot\main.py --max-nodes 200 --limit 6 --sum-node 200
```

跳过 embedding matrix，只看随机树：

```powershell
python Tree_plot\main.py --max-nodes 200 --limit 6 --no-matrix
```

不保存 PNG，只显示窗口：

```powershell
python Tree_plot\main.py --max-nodes 120 --no-save
```
...
```powershell
python Tree_plot\main.py --sum-node 8 --count-only
```
...
```powershell
python Tree_plot\main.py --sum-node 50 --count-only
python Tree_plot\main.py --sum-node 200 --count-only
```
...
CPU 枚举的思路是“把每棵树都造出来”。这适合小规模，因为能看到真实对象；但大规模时，它会被大量重复结构和组合数淹没。
